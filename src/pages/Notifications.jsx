...
const Notifications = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const userIds = [
        ...new Set((data || []).map((n) => n.related_user_id).filter(Boolean)),
      ];
      let profiles = [];
      if (userIds.length > 0) {
  ...
  return (
    <DashboardLayout>
      <SEOHead
        title="Notifications"
        description="Stay updated with your latest notifications on BizBase AI."
        path="/notifications"
        noIndex
      />
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-foreground flex items-center gap-2 text-base">
                <UserPlus className="w-5 h-5 text-purple-600" /> Connection
                Requests
              ...
              {connectionRequests.map((req) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {req.profile.current_position}
                        {req.profile.company_name
                          ? ` at ${req.profile.company_name}`
                          : ""}
                      </p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      onClick={() => handleConnectionRequest(req.id, "accept")}
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleConnectionRequest(req.id, "reject")}
                    >
                      <X className="w-4 h-4" />
                    </Button>
                  </div>
                  ...
                  >
                    <Avatar className="h-9 w-9 flex-shrink-0">
                      ...
                      <AvatarFallback className="bg-primary/10 text-primary text-xs">
                        {n.related_user?.full_name?.charAt(0) || "?"}
                      </AvatarFallback>
                    </Avatar>
