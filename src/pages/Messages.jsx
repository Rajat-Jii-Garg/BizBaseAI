...
const Messages = () => {
  ...
  const incomingMessagesRealtimeRef = useRef(null);
  ...
  const setupConversationsRealtime = useCallback(() => {
    ...
    conversationsRealtimeRef.current = conversationChannel;
    ...
    const incomingChannel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          console.log("Incoming realtime message:", message.id);
          ...
          if (isCurrentConversation) {
            try {
              ...
              console.error("Read acknowledgement failed:", error);
            }
            ...
            if (
              ...
              try {
                new Notification("New message on BizBase", {
                  body: String(
                    message.content || "You received a new message",
                  ).slice(0, 100),
                  icon: "/favicon.ico",
                  tag: `bizbase-message-${message.conversation_id}`,
                });
              } catch (error) {
      ...
      .subscribe((status) => {
        console.log("Incoming message realtime:", status);
      });

    incomingMessagesRealtimeRef.current = incomingChannel;
  }, [user?.id, toast]);
  ...
  const setupCallListener = () => {
    ...
    ch.on("broadcast", { event: "answer" }, async ({ payload }) => {
      ...
      if (webrtcManagerRef.current && payload.answer) {
        await webrtcManagerRef.current.handleAnswer({
  ...
  const initiateCall = async (type) => {
    ...
    try {
      const { stream, offer } = await webrtcManagerRef.current.startCall(
        type === "video",
      );
  ...
  const handleAcceptCall = async () => {
    ...
    if (pendingRemoteIceRef.current.length > 0) {
      console.log(
        ...
        "queued ICE candidates to manager",
      );
  ...
  const sendMessage = async () => {
    ...
    if (!otherParticipant?.id) {
      toast({
        title: "Error",
        description: "Recipient could not be determined.",
        variant: "destructive",
      });
    ...
    try {
      ...
      /*
       * Secure server-side message creation.
       */
      ...
      /*
       * Realtime will also deliver this,
       * but add returned DB row immediately.
       */
      if (sentMessage) {
        setMessages((prev) => {
      ...
      /*
       * Conversation timestamp is now
       * automatically updated by DB trigger.
       *
       * NO client-side conversation UPDATE.
       */
      fetchConversations();
    } catch (error) {
  ...
  const startConversation = async (selectedUser) => {
    ...
    try {
       ...
       *
       * Prevents duplicate conversation
       * race conditions.
       */
       ...
       */
      const { data: fullConversation, error: conversationError } =
        await supabase
          .from("conversations")
          .select(
            `
      ...
      setConversations((prev) => {
        const withoutCurrent = prev.filter(
          (item) => item.id !== fullConversation.id,
        );

        return [fullConversation, ...withoutCurrent];
      });
  ...
  return (
    <DashboardLayout>
      <SEOHead
        title="Messages"
        description="Chat with your professional connections on BizBase AI."
        path="/messages"
        noIndex
      />
                ...
                <div className="flex items-center justify-center h-full bg-gradient-to-br from-accent/10 to-accent/5">
                  <div className="text-center max-w-md px-4">
                    <div className="mb-6 relative">
                      ...
                      <MessageCircle
                        className="h-20 w-20 text-primary/60 mx-auto relative"
                        strokeWidth={1.5}
                      />
                    </div>
